...
class Pooling:
    ...
    def forward(self,X):
        self.X = X
        self.Y = 0.5*(X[:,:-1:2,:-1:2,:]+X[:,:-1:2,1::2,:]+X[:,1::2,:-1:2,:]+X[:,1::2,1::2,:])
        return self.Y
...
class Convolution:
    ...
    def __init__(self,name,write=False):
        ...
        if write:
            ...
            self.W = numpy.random.normal(0,1/(w*h*m)**.5,wshape); self.DW = self.W*0
            self.B = numpy.zeros([n]); self.DB = self.B*0
    ...
    def update(self,lr):
        ...
        for i in range(ww):
            for j in range(hw):
                ...
                dy = self.DY
                self.DW[i,j,:,:] += numpy.tensordot(x,dy,axes=([0,1,2],[0,1,2])) / len(self.X)

        self.DB = 0.9*self.DB + self.DY.sum(axis=(0,1,2)) / len(self.X)
